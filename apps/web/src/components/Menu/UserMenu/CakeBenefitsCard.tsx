...
const CakeBenefitsCard: React.FC<React.PropsWithChildren<CakeBenefitsCardProps>> = ({ onDismiss }) => {
  ...
  const {
    ...
    <>
      <Text>{t(`bCAKE allows you to boost your yield in PlgSwap Farms by up to 2x.`)}</Text>
  ...
  const {
    ...
    <>
      <Text>
        {t(`vCAKE boosts your voting power to %totalScore% in the PlgSwap voting governance.`, {
