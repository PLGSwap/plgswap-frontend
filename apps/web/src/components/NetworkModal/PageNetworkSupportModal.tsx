import { Button, Modal, Text, Grid, Box, Message, MessageText } from '@plgswap/uikit'
  ...
  return (
    <Modal title={title || t('Check your network')} hideCloseButton headerBackground="gradientCardHeader">
      <Grid style={{ gap: '16px' }} maxWidth="360px">
        <Text bold>{t('It’s a POLYGOMIC Smart Chain only feature')}</Text>
        ...
        <Text small>
          {t(
            'Our Pools, Limit, Trading Competition, Prediction, Lottery and NFTs features are currently available only on POLYGOMIC Chain! Come over and join the community in the fun!',
          )}
        </Text>
        {canSwitch ? (
          <Button
          ...
          >
            {t('Switch to %chain%', { chain: 'POLYGOMIC Smart Chain' })}
          </Button>
        ) : (
